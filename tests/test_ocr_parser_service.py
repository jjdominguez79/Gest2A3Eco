"""Tests unitarios de InvoiceInterpreter y sus funciones auxiliares puras."""
import pytest

from services.ocr.invoice_interpreter import (
    InvoiceInterpreter,
    _detectar_nif,
...
def test_parsear_factura_completa_pendiente_revision():
    r = InvoiceInterpreter().interpretar(FACTURA_COMPLETA)
...
def test_parsear_sin_nif_produce_error():
    r = InvoiceInterpreter().interpretar(FACTURA_SIN_NIF)
    assert r.estado_sugerido == OcrDocumentState.ERROR
    assert any("NIF" in e for e in r.errores)


def test_parsear_sin_numero_factura_produce_error():
    r = InvoiceInterpreter().interpretar(FACTURA_SIN_NUMERO)
    assert r.estado_sugerido == OcrDocumentState.ERROR
    assert any("factura" in e.lower() for e in r.errores)


def test_parsear_texto_vacio_produce_error():
    r = InvoiceInterpreter().interpretar("")
    assert r.estado_sugerido == OcrDocumentState.ERROR
    assert r.errores


def test_parsear_multibase_tabla_produce_dos_lineas():
    r = InvoiceInterpreter().interpretar(FACTURA_MULTIBASE_TABLA)
    assert r.estado_sugerido == OcrDocumentState.PENDIENTE_REVISION
    assert len(r.bases_iva) == 2
    assert {linea.tipo_iva for linea in r.bases_iva} == {10.0, 21.0}


def test_parsear_multibase_tabla_suma_bases():
    r = InvoiceInterpreter().interpretar(FACTURA_MULTIBASE_TABLA)
    assert r.base_total == pytest.approx(1000.0, abs=0.01)
    assert r.iva_total == pytest.approx(166.0, abs=0.01)
...
def test_detectar_pares_tipo_iva_directo():
    ...
    assert len(lineas) == 2
    by_tipo = {linea.tipo_iva: linea for linea in lineas}
    assert by_tipo[21.0].base == 1000.0
    assert by_tipo[21.0].cuota_iva == 210.0
    assert by_tipo[10.0].base == 500.0
    assert by_tipo[10.0].cuota_iva == 50.0


def test_total_incoherente_produce_error_de_validacion():
    texto = """\
    Empresa ABC SA
    CIF: B11223344
    Factura N: F-001
    ...
    """
    r = InvoiceInterpreter().interpretar(texto)
    assert r.estado_sugerido == OcrDocumentState.PENDIENTE_REVISION
    assert any("incoher" in error.lower() for error in r.errores)


def test_total_cero_no_bloquea_si_campos_minimos_estan():
    texto = """\
    Empresa XYZ SL
    NIF: B55667788
    Factura N: F-002
    ...
    assert r.estado_sugerido == OcrDocumentState.PENDIENTE_REVISION
    assert r.total == 0.0
    assert any("total" in error.lower() for error in r.errores)


def test_nif_no_cruza_salto_de_linea():
    r = InvoiceInterpreter().interpretar(TEXTO_NIF_LABEL_LUEGO_OTRO_CAMPO)
...
def test_to_dict_contrato_tipado():
    r = InvoiceInterpreter().interpretar(FACTURA_COMPLETA)
    d = r.to_dict()
    for campo in (
        "proveedor_nif", "numero_factura", "fecha_factura",
        "base_total", "iva_total", "total", "bases_iva", "errores",
    ):
        assert campo in d, f"Campo '{campo}' ausente en to_dict()"
    assert isinstance(d["bases_iva"], list)
