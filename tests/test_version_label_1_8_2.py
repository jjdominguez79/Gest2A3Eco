"""
Tests de version y etiqueta de pantalla de acceso.

Cubre:
1. APP_VERSION tiene semver valido.
...
def test_app_version_tiene_semver_valido():
...
def test_get_version_label_formato_completo():
    ...
    assert f"v{APP_VERSION}" in label
...
def test_ui_auth_no_tiene_version_hardcodeada():
    ...
    assert APP_VERSION not in contenido, (
        "views/ui_auth.py no debe contener la version actual hardcodeada"
    )
