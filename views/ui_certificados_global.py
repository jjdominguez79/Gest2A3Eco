...
def _fecha_caducidad_orden(valor) -> date | None:
    """Normaliza la fecha de caducidad para poder ordenarla cronologicamente."""
...
def _ordenar_por_caducidad(certificados: list[dict], descendente: bool = False) -> list[dict]:
    """Ordena por fecha real y deja las fechas ausentes o invalidas al final."""
    con_fecha: list[tuple[date, int, dict]] = []
    sin_fecha: list[dict] = []
    for posicion, certificado in enumerate(certificados):
        fecha = _fecha_caducidad_orden(certificado.get("fecha_caducidad"))
        if fecha is None:
            sin_fecha.append(certificado)
        else:
            con_fecha.append((fecha, posicion, certificado))
    con_fecha.sort(key=lambda item: (item[0], item[1]), reverse=descendente)
    return [item[2] for item in con_fecha] + sin_fecha
...
class UICertificadosGlobal(ttk.Frame):
    ...
    def _render(self) -> None:
        ...
        rows_mostradas = len(certificados_filtrados)
